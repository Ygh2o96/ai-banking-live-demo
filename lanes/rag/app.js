...
const H1_GENESIS_STATES = ["genesis", "seed-skeleton", "seed-binding", "seed-living", "seed-models"];
...
const LATTICE_FUNDING_BIN_LABELS = Object.freeze({
  zero: "零外部融资",
  "gt-0-le-10": "外部融资 > HK$0m–HK$10m",
  "gt-10-le-25": "外部融资 > HK$10m–HK$25m",
  "gt-25-le-50": "外部融资 > HK$25m–HK$50m",
});
...
function renderH1Model(model, title, baseline = demoData.hero1.baseline) {
  ...
  $("#h1-replay").textContent = `可回溯验证 ${model.replay_hash.slice(0, 10)}`;
...
function activateH1View(view, updateUrl = false) {
  ...
  $$('[data-h1-view]').forEach((button) => {
    const selected = button.dataset.h1View === panelView;
  ...
  const titles = {
    ...
    genesis: "模型生成路径 · 两种受控构建路径",
    "seed-skeleton": "JSON seed 骨架 · 可复用模型语法，不含企业数据",
    "seed-binding": "Slot 概念绑定 · 明确数据来源与流转，不掩盖冲突",
    "seed-living": "动态模型 · 公式上下游关系与勾稽检查",
    "seed-models": "三种模型架构 · Banker 专业判断与选择",
  };
  $("#h1-view-title").textContent = titles[view];
  if (genesisMode) {
    if (!window.MODEL_GENESIS) throw new Error("Seed 集成模块不可用");
...
function renderWiggleTrace() {
  ...
  $("#h1-wiggle-view").innerHTML = `<div class="wiggle-layout"><section class="wiggle-graph ruled-panel"><div class="evidence-heading wiggle-heading"><div><span>影响传导追踪 / ${escapeHtml(trace.event_id)}</span><strong>单项假设变动；非受影响下游项目保持不变</strong><small>联动有效性测试，非最终可行方案；后续进入求解空间。</small></div><div class="trace-legend"><span class="legend-move">必须联动</span><span class="legend-still">保持不变</span><span class="legend-control">勾稽检查</span></div></div><svg id="wiggle-graph" viewBox="0 0 1160 500" role="img" aria-label="DIO wiggle 影响传导图"><defs><marker id="arrow-must_move" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z"/></marker><marker id="arrow-must_not_move" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z"/></marker><marker id="arrow-control_check" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z"/></marker></defs>${edges}${nodes}</svg></section><aside class="trace-register ruled-panel"><div class="evidence-heading"><div><span>数据底稿数值</span><strong>精准生成数值</strong></div></div><div class="trace-register-body">${exactRows}</div><div class="trace-reconcile"><span>节点 / 连线规则</span><strong>${integer(trace.nodes.length)} 个节点 · ${integer(trace.edges.length)} 条有向边</strong><small>${escapeHtml(trace.source_output)} · ${escapeHtml(trace.visualization_hash.slice(0, 12))}</small></div><button id="wiggle-change-log" class="secondary-action">查看完整变动日志</button></aside></div>`;
...
function renderDriverList(model, baseline) {
  ...
  const html = names.map(([key, label, kind]) => {
    const value = model.drivers[key]["2028E"];
    const old = baseline.drivers[key]["2028E"];
...
function renderStatementTable() {
  ...
  if (currentStatement === "pnl") {
    rows = [
      ["营业收入 Revenue", "revenue", true],
      ["毛利 Gross profit", "gross_profit", false],
      ["EBITDA", "ebitda", false],
      ["净利润 Net profit", "net_profit", true],
    ];
  } else if (currentStatement === "cfs") {
    rows = [
      ["经营活动现金流 CFO", "cfo", false],
      ["投资活动现金流 CFI", "cfi", false],
      ["筹资现金流 / 外部融资 CFF", "cff", false],
      ["期末现金余额 Closing cash", "closing_cash", true],
    ];
  } else {
    rows = [
      ["货币资金 Cash", "cash", true],
      ["应收账款 Accounts receivable", "accounts_receivable", false],
      ["存货 Inventory", "inventory", false],
      ["应付账款 Accounts payable", "accounts_payable", false],
      ["有息负债 Debt", "debt", false],
      ["资产负债平衡检查 BS check", "check", true],
    ];
  }
  const values = (year, key) => {
    if (currentStatement === "pnl") return currentModel.annual[year][key];
    if (currentStatement === "cfs") {
      if (key === "closing_cash") return [...currentModel.months].reverse().find((row) => row.year === year).bs.cash;
      return currentModel.annual[year][key];
    }
    return [...currentModel.months].reverse().find((row) => row.year === year).bs[key];
  };
  $("#statement-table").innerHTML = `<table class="financial-table"><thead><tr><th>百万元港币 HK$m</th>${years.map((year) => `<th>${year}</th>`).join("")}</tr></thead><tbody>${rows.map(([label, key, total]) => `<tr class="${total ? "total" : ""}"><td>${label}</td>${years.map((year) => `<td>${number(values(year, key), 2)}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
}
...
function resetPrecedent() {
  ...
  $("#precedent-title").textContent = "先把关键词查准";
...
function runKeyword() {
  ...
  $("#precedent-title").textContent = "关键词先找字面相近项，再剔除看似相关的误导项";
  $("#embedding-run").disabled = false;
  $("#matrix-state").textContent = "第一组证据";
  toast("关键词结果混入了实为代销模式的误导案例，须由 Banker 人工剔除。", "amber");
}
...
function runEmbedding() {
  ...
  $("#precedent-title").textContent = "语义检索找回换了说法的先例；是否适用，仍看招股书原文";
  renderMatrix();
}
